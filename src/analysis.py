...
def calculate_annual_stats() -> None:
    ...
    This function computes the average maximum temperature, average minimum temperature,
    and total precipitation for each weather station and year. The results are stored
    in the WeatherStats table. If a statistic cannot be calculated, NULL is used.
    """
    ...
    with app.app_context():
        ...
        stats: List[Tuple] = db.session.query(
            WeatherData.station_id,
            extract('year', WeatherData.date).label('year'),
            func.avg(case((WeatherData.max_temp.isnot(None), WeatherData.max_temp))).label('avg_max_temp'),
            func.avg(case((WeatherData.min_temp.isnot(None), WeatherData.min_temp))).label('avg_min_temp'),
            func.sum(case((WeatherData.precipitation.isnot(None), WeatherData.precipitation))).label('total_precipitation')
        ).group_by(
            WeatherData.station_id,
            extract('year', WeatherData.date)
        ).all()
        
        logger.info(f"Calculated statistics for {len(stats)} station-year combinations")
        ...
        db.session.commit()
        logger.info("Annual statistics calculation completed and stored in database")
